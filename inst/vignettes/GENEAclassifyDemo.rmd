...
This guide on using RStudio with GitHub is helpful <https://www.r-bloggers.com/rstudio-and-github/>.
...
Inside GENEAclassify there is also a range of custom functions:
 ...
 - CirDisp (circular dispertion for radians)
 - CirSkew (circular skewness for radians)
 - CirKurt (circular kurtosis for radians)
 - impact (calcutes the proportion of samples above a defined absolute magnitude)
...
The segmentation function also applies a default step counting algorithm when no arguments are passed through the function. The step counting algorithm works by taking the y axis, filtering the signal with a chebyshev filter, applying a hystersis threshold where the zero crossing are counted over a given window. 
